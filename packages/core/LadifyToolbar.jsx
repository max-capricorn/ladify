...
export class LadifyToolbar extends React.Component {
  ...
  changeId = (e, l) => {
    ...
    if (!newi) return;
    // check if id 重复
    const duplicatedAry = this.state.widgets.filter((w) => w.i === newi);
    ...
    this.setState(
      produce(({ layers, layerLevel }) => {
        Object.keys(layers[layerLevel].layouts).forEach((k) => {
          const targetW = layers[layerLevel].layouts[k].filter(
            (m) => m.i === oldi
          );
    ...
    return (
      ...
      widgets.map((l, i) => {
        if (this.importedWidgets[l.type]) {
          const h = this.importedWidgets[l.type].getCellH() || 1;
          const w = this.importedWidgets[l.type].getCellW() || 4;
          const rdata = { logic: this.props.logic, l };
          return (
            <div
              key={l.i}
  ...
  isPointInBB(p, bb) {
    return (
      p.x >= bb.x && p.x <= bb.x + bb.w && p.y >= bb.y && p.y <= bb.y + bb.h
    );
  }
  ...
  markWidgets() {
    ...
    const gridWitdth =
      this.containerRef.current.clientWidth - this.gridPadding * 2;
    ...
    layoutedWidgets.map((item) => {
      ...
      const wcenter = {
        x: widgetX + widgetWidth / 2,
        y: widgetY + widgetHeight / 2,
      };
      ...
      if (this.isPointInBB(wcenter, bigBounding)) {
        layers[layerLevel].widgets = layers[layerLevel].widgets.map((w) => {
          if (w.i === item.i) {
            w.selected = true;
          }
          return w;
  ...
  mouseDown(e) {
    ...
    const p = this.getRelativeXY(e);
    const results = this.cur_widgets_cords.filter((w) =>
      this.isPointInBB(p, w)
    );
    if (results.length > 0) {
  ...
  onRemoveItem(i) {
    const { layers, layerLevel } = this.state;
    layers[layerLevel].widgets = layers[layerLevel].widgets.filter(
      (item, index) => index !== i
    );
    this.setState([...layers]);
  ...
  updateWidgetsCord(layouts) {
    ...
    layoutedWidgets.map((cell) => {
      const item = this.cord_grid2px(cell);
  ...
  render() {
    const { layerLevel, layers } = this.state;
    const { pageId } = this.props;
    const { layouts } = layers[0];
    const editorDidMount = async (editor, monoco) => {
      this.editor = editor;
      const code = await service.getcode(this.props.pageId);
    ...
    return (
      ...
      >
        <Content style={{ marginTop: 44, marginBottom: 100 }}>
          ...
          <div className="first" onClick={() => this.goFirstLayer()}>
            1
          </div>
          <div className="next" onClick={() => this.goNextLayer()}>
            2
          </div>
          {/* <Button
              type="ghost"
            ...
            {this.state.debug ? (
              <>
                ...
                {Object.keys(this.importedWidgets).map((k) => (
                  ...
                  >
                    {this.importedWidgets[k].getWidgetType()}
                  </Button>
                ))}
              </>
            ) : (
