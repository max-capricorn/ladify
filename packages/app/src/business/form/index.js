...
export default class FormLogic extends LadifyPageContext {
  constructor() {
    ...
    effect(() => {
      ...
      if (this.s.price > 15) this.setState('btn:warning', 'type', 'danger');
      else this.setState('btn:warning', 'type', 'default');
    });
  }
  ...
  componentDidMount(id) {
    ...
    if (id === 'btn:plus') {
      this.setState(id, 'content', 'plus');
      return true;
    }
    if (id === 'btn:minus') {
      this.setState(id, 'content', 'minus');
      return true;
    }
  }
