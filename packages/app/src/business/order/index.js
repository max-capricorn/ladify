  ...
  handleEvent(id, type, payload) {
    if (type === 'menu.onClick') {
      ...
      switch (id) {
        ...
        case 'form_btn':
          const that = this;
          function getDropValue(ids) {
            const menus = that.getState(ids, 'menus');
            const idx = that.getState(ids, 'selected');
            return menus[idx];
          }
          alert(
            JSON.stringify({
          ...
          break;
        case '9':
          const form = [1, 2, 3, 4, 5, 6, 7, 8, 9].reduce((o, i) => {
            o[i] = this.getState(i, 'value');
            return o;
          }, {});
          setTimeout(() => {
